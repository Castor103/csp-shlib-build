...
int main(int argc, char * argv[]) {
    ...
    if (kiss_device) {
        csp_usart_conf_t conf = {
            ...
            .paritysetting = 0,
            .checkparity = 0};
        error = csp_usart_open_and_add_ewc_interface(&conf, CSP_IF_EWC_DEFAULT_NAME,  &default_iface);
        if (error != CSP_ERR_NONE) {
...
#if (CSP_HAVE_LIBZMQ)
    if(zmq_device) {
        error = csp_zmqhub_init(255, zmq_device, 0, &default_iface);
        if (error != CSP_ERR_NONE) {
    ...
    // Set CSP Packet Buffer
    packet_send = csp_buffer_get(0 + sizeof *packet_send);

    /* Wait for execution to end (ctrl+c) */
    while(1) {
        // Copy Data to CSP Packet
        // int result = csp_ping(1, 1000, 100, CSP_O_NONE);
		// csp_log_info("Ping address: %u, result %d [mS]", 1, result);
        // csp_sleep_ms(1000);
        // result = csp_ping(2, 1000, 100, CSP_O_NONE);
		// csp_log_info("Ping address: %u, result %d [mS]", 2, result);
        // csp_sleep_ms(1000);
        // result = csp_ping(3, 1000, 100, CSP_O_NONE);
		// csp_log_info("Ping address: %u, result %d [mS]", 3, result);
        // csp_sleep_ms(1000);
        // result = csp_ping(4, 1000, 100, CSP_O_NONE);
		// csp_log_info("Ping address: %u, result %d [mS]", 4, result);
        // csp_sleep_ms(1000);
    }
